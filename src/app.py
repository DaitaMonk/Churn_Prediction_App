...
with gr.Blocks(theme=gr.themes.Base(primary_hue="stone", neutral_hue="stone")) as block:
    ...
    with gr.Row():
        with gr.Column():
            SeniorCitizen = gr.Radio(
                ["Yes", "No"], label="Are you a Senior Citizen?")
            Partner = gr.Radio(["Yes", "No"], label="Do you have a partner?")
            Dependents = gr.Radio(
                ["Yes", "No"], label="Do you have dependents?")
            tenure = gr.Number(
                label="Tenure (months): How long have you been at the company", minimum=0)
            InternetService = gr.Radio(
                ["DSL", "Fiber optic", "No"], label="What Internet Service Do You Use?")
            OnlineSecurity = gr.Radio(
                ["Yes", "No", "No internet service"], label="Do You Have Online Security?")

        with gr.Column():
            OnlineBackup = gr.Radio(
                ["Yes", "No", "No internet service"], label="Do You Have Any Online Backup Service?")
            DeviceProtection = gr.Radio(
                ["Yes", "No", "No internet service"], label="Do You Use Any Device Protection?")
            TechSupport = gr.Radio(
                ["Yes", "No", "No internet service"], label="Do You Use TechSupport?")
            StreamingTV = gr.Radio(
                ["Yes", "No", "No internet service"], label="Do You Stream TV?")
            StreamingMovies = gr.Radio(
                ["Yes", "No", "No internet service"], label="Do You Stream Movies?")

        with gr.Column():
            Contract = gr.Radio(
                ["Month-to-month", "One year", "Two year"], label="What Is Your Contract Type?")
            PaperlessBilling = gr.Radio(
                ["Yes", "No"], label="Do You Use Paperless Billing?")
            PaymentMethod = gr.Dropdown(["Electronic check", "Mailed check", "Bank transfer (automatic)",
                                        "Credit card (automatic)"], label="What Payment Method Do You Use?")
            MonthlyCharges = gr.Number(
                label="What is your Monthly Charge?", minimum=0)
            TotalCharges = gr.Number(
                label="What are your Total Charges?", minimum=0)
    ...
    gr.Examples([['No', 'No', 'No', '12', 'Fiber optic', 'No', 'No', 'No', 'No', 'Yes', 'No', 'Month-to-month', 'Yes', 'Electronic check', '84.45', '1059.55'],
                 ...
                 ['No', 'No', 'No', '9', 'No', 'No internet service', 'No internet service', 'No internet service', 'No internet service',
                     'No internet service', 'No internet service', 'Month-to-month', 'No', 'Mailed check', '20.40', '181.80'],
